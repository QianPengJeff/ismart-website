...
if ( ! function_exists( 'themify_do_img' ) ) {
	...
	function themify_do_img( $img_url = null, $width, $height, $crop = false ) {
		...
		// Save value that indicates if we'll be using the large size
		...
		if ( is_array( $meta ) && isset( $meta['sizes'] ) && is_array( $meta['sizes'] ) ) {
			if ( is_null( $height ) || 0 === $height || '0' === $height ) {
				if ( $use_large_not_full && isset( $meta['sizes']['large']['file'] ) ) {
					$height = isset( $meta['sizes']['large']['height'] ) ? $meta['sizes']['large']['height'] : 0;
				} else {
			...
			foreach( $meta['sizes'] as $key => $size ) {
				if ( $size['width'] == $width && $size['height'] == $height ) {
					...
					return array(
						'url' => str_replace( basename( $img_url ), $size['file'], $img_url ),
						'width' => $width,
						'height' => $height,
					);
				}
		...
		$image = themify_make_image_size( $attachment_id, $width, $height, $meta, $img_url, $use_large_not_full );
		if ( true == $crop ) {
...
if ( ! function_exists( 'themify_make_image_size' ) ) {
	 ...
	 * @param array $meta
	 * @param string $img_url
	 * @param bool $use_large_not_full
	 *
	 * @return array
	 */
	function themify_make_image_size( $attachment_id, $width, $height, $meta, $img_url, $use_large_not_full ) {
		setlocale( LC_CTYPE, get_locale() . '.UTF-8' );
