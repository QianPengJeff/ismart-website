...
function themify_theme_fixed_header() {
	...
	if ( in_array( $header, array( 'header-leftpane', 'header-minbar', 'boxed-content', 'none') ) ) {
		return '';
	}
...
if ( ! function_exists( 'themify_theme_init_types' ) ) {
	...
	function themify_theme_init_types() {
		...
		themify_build_write_panels( apply_filters( 'themify_theme_meta_boxes', array(
			...
			array(
				'name'    => __( 'Theme Appearance', 'themify' ),
...
if ( ! function_exists( 'themify_theme_custom_post_css' ) ) {
	...
	function themify_theme_custom_post_css() {
		...
		if ( in_array( get_post_type(), array( 'post', 'page', 'portfolio' ) ) ) {
			...
			if ( is_singular() && 'color-custom' == themify_theme_get( 'color_scheme_mode', 'color-presets' ) ) {
...
function themify_theme_header_background( $area = 'header', $classes = '' ) {
	...
	$post_id = get_the_ID();
	$nobg = '';
	$header_wrap = themify_get( 'header_wrap' );
	$image_meta = '';
	$background_gallery = '';
	
	if ( 'header' === $area && in_array( $header_wrap, array( 'video', 'slider' ) ) ) {
		...
		if ( is_page() || is_singular( 'post' ) || is_singular( 'portfolio' ) ) {
			...
			if ( $gallery = ( ( $header_wrap == 'slider' ) || ( $header_wrap == '' && $background_gallery != '' ) ) ) {
	...
	if ( is_singular() && ! is_attachment() ) {
		$solid_color = get_post_meta( $post_id, 'background_color', true );
		$solid_image = get_post_meta( $post_id, 'background_image', true );
		if ( 'colors' == $header_wrap 
			|| ( 'solid' == $header_wrap && '' == $solid_image && '' == $solid_color )
			|| ( 'slider' == $header_wrap && empty( $background_gallery ) )
			|| ( 'video' == $header_wrap && empty( $video ) )
			|| is_null( $header_wrap )
		) {
			// Set first color as initial background-color to avoid a color jump
			$colors = themify_theme_animated_bg_default_colors();
			$first_color = themify_check( 'setting-color_stop_1' ) ? themify_get( 'setting-color_stop_1' ) : $colors[0];
			if ( themify_theme_animated_bg_check( $area ) ) {
				$classes .= ' animated-bg';
				$style .= 'background-color: #' . str_replace( '#', '', $first_color ) . ';';
			}
		}
	} else {
		// Set first color as initial background-color to avoid a color jump
...
class Themify_Background_Gallery{
	...
	function create_controller(){
		...
		$bggallery_enable = ((get_post_meta(get_the_ID(), 'header_wrap', true) == 'slider') || (get_post_meta(get_the_ID(), 'header_wrap', true) == '' && get_post_meta(get_the_ID(), 'background_gallery', true) != ''));
